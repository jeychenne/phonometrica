...
matter what the type of the parameter is. This makes it possible to pass null values to functions to signal that the value is invalid. 
...
Note: creating a named function and assigning a function expression to a variable are strictly equivalent in the top-level scope, but they are slightly different
when they are created in an embedded scope: variables are always global, unless they are declared with the keyword ``let``, whereas functions are local to the scope,
