...
- **Statistical modeling** *(preview)*: fit linear, logistic, Poisson, and negative binomial regression models with fixed and random effects (LMM/GLMM); post-hoc testing (estimated marginal means); model comparison with LRT and information criteria; diagnostic plots and exploratory visualizations. Validated against reference implementations in R, but offered as a preview: check results you intend to publish against R, and please report any discrepancy.
...
All other dependencies (Eigen, PCRE2, pocketfft, CppAD, etc.) are bundled in the source tree. See [BUILD.md](BUILD.md) for detailed instructions.
