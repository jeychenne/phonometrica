...
#include <phon/analysis/praat_pitch.cpp>
...
#include <phon/third_party/sptk4/include/SPTK/analysis/pitch_extraction_by_rapt.h>
#include <phon/third_party/swipe/swipe.h>
#include <phon/third_party/Eigen/unsupported/Eigen/Polynomials>
...
static std::vector<double> lpc_burg(const Array<double> &x, int order)
{
    ...
    //            A New Autoregressive Spectrum Analysis PitchTracker.
...
bool get_formants(const std::vector<double> &lpc_coeffs, double Fs, std::vector<double> &freqs, std::vector<double> &bw)
{
	...
	auto coeffs = Eigen::VectorXd::Map(lpc_coeffs.data(), lpc_coeffs.size());
	Eigen::PolynomialSolver<double, Eigen::Dynamic> solver;
	solver.compute(coeffs);
	auto &root_candidates =  solver.roots();
...
std::vector<double> get_pitch(PitchTracker algorithm, const Array<double> &input, double sample_rate, double min_pitch, double max_pitch, double time_step, double voicing_threshold, double octave_jump_cost, double voicing_cost)
{
	if (algorithm == PitchTracker::Praat)
	{
		return get_pitch_praat(input, sample_rate, min_pitch, max_pitch, time_step, voicing_threshold, octave_jump_cost, voicing_cost);
	}
	...
	if (algorithm == PitchTracker::Swipe)
	{
		...
		vec.v = const_cast<double*>(input.data());
	...
	// From SPTK
	//if t is raised, the number of voiced frames increase in RAPT, REAPER, DIO, and Harvest
	//if t is dropped, the number of voiced frames increase in SWIPE
	// Voicing thresholds:
	...
	std::vector<double> waveform(input.begin(), input.end()), f0;
	bool needs_scaling = true; // REAPER and RAPT need values scaled to 16 bits
	int frame_shift = static_cast<int>(time_step * sample_rate);
	std::unique_ptr<sptk::PitchExtractionInterface> tracker;

	if (algorithm == PitchTracker::Harvest)
	{
		tracker = std::make_unique<sptk::PitchExtractionByHarvest>(frame_shift, sample_rate, min_pitch, max_pitch, voicing_threshold);
		needs_scaling = false;
	}
	else if (algorithm == PitchTracker::Reaper)
	{
		tracker = std::make_unique<sptk::PitchExtractionByReaper>(frame_shift, sample_rate, min_pitch, max_pitch, voicing_threshold);
	}
	else
	{
		tracker = std::make_unique<sptk::PitchExtractionByRapt>(frame_shift, sample_rate, min_pitch, max_pitch, voicing_threshold);
	}
	...
	if (!tracker->Get(waveform, &f0, nullptr, nullptr))
	{
		throw error("Pitch tracking failed\n");
	}
