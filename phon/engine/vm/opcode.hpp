...
#define PHON_OPCODE_LIST(_)                                                     \
	...
	/* first-class references (`ref` params, design/references.md) */           \
	_(MAKEREF)     /* A B    : R[A] = boxed reference promoting local R[B]    */ \
	...
	_(GETIDXN)     /* A B C  : R[A] = R[B][R[B+1..B+C]]  (NumArray, C indices) */ \
	...
	_(GETVIEW)     /* A B C  : R[A] = view of NumArray R[B]; C axes;3 regs/axis */ \
