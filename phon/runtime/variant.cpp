...
String Variant::class_name() const
{
	...
	switch (data_type())
	{
		...
		case Datatype::Object:
		{
			if (check_type<Instance>(*this))
			{
				auto &i = raw_cast<Instance>(*this);
				return i.get_class()->name();
			}
			else
			{
...
bool Variant::operator==(const Variant &other) const
{
	...
	if (v1.data_type() == v2.data_type())
	{
		switch (v1.data_type())
		{
			...
			case Datatype::Object:
			{
				if (check_type<Instance>(v1))
				{
					auto &i1 = raw_cast<Instance>(v1);
					auto &i2 = raw_cast<Instance>(v2);
	...
	if (v1.data_type() == v2.data_type())
	{
		switch (v1.data_type())
		{
			...
			case Datatype::Object:
			{
				...
				if (o1->get_runtime_class() != o2->get_runtime_class()) {
					break;
				}
...
String Variant::as_string() const
{
	switch (m_data_type)
	{
		...
		case Datatype::Float:
		{
			...
			double num2 = floor(num);
			if (num == num2) {
				return meta::to_string(intptr_t(num2));
			}
			return meta::to_string(num);
		}
		case Datatype::Boolean:
...
Class *Variant::get_class() const
{
	switch (data_type())
	{
		...
		case Datatype::Object:
			return as.object->get_runtime_class();
		case Datatype::Alias:
...
Variant & Variant::unshare()
{
	...
	// If an instance is a reference type, don't clone. Otherwise, we let it fall through and it will get automatically
	// cloned as an object.
	if (check_type<Instance>(*this) && !raw_cast<Instance>(*this).clonable()) {
		return *this;
	}
	switch (data_type())
	{
