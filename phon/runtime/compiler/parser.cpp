...
String Parser::this_keyword("this");
...
AutoAst Parser::parse_statement()
{
	...
	else if (accept(Lexeme::Local))
	{
		if (accept(Lexeme::Let)) {
			report_error("Cannot use 'local' before 'let' since 'let' already creates a locally scoped variable");
		}
...
AutoAst Parser::parse_formula()
{
	trace_ast();
	if (accept(Lexeme::Tilde))
	{
		return make<BinaryExpression>(Lexeme::Tilde, nullptr, parse_comp_expression());
	}

	auto e = parse_comp_expression();

	if (accept(Lexeme::Tilde))
	{
		return make<BinaryExpression>(Lexeme::Tilde, std::move(e), parse_comp_expression());
	}
...
AutoAst Parser::parse_pipe_expression()
{
	trace_ast();
	auto e = parse_call_expression();

	if (accept(Lexeme::Pipe))
	{
		return make<BinaryExpression>(Lexeme::Pipe, std::move(e), parse_identifier("in formula"));
	}
...
{
	trace_ast();
	auto e = parse_pipe_expression();

	while (accept(Lexeme::OpPower))
	{
		e = make<BinaryExpression>(Lexeme::OpPower, std::move(e), parse_pipe_expression());
	}
	return e;
}
...
AutoAst Parser::parse_primary_expression()
{
	...
	else if (check(Lexeme::This))
	{
		if (!parsing_class) {
			report_error("[Syntax error] Cannot use \"this\" outside of a class");
		}
		auto name = token.spelling;
		accept();
		return make<Variable>(std::move(name));
	}
...
AstList Parser::parse_parameters(bool method)
{
	trace_ast();
	AstList params;
...
AutoAst Parser::parse_class_declaration(bool local)
{
	...
	{
		auto super = token.spelling;
		parent = parse_identifier(hint);
		if (class_name == super) {
	...
	while (!check(Lexeme::End))
	{
		...
		else if (accept(Lexeme::Method))
		{
			methods.push_back(parse_function_declaration(true, true));
			auto r = dynamic_cast<RoutineDefinition*>(methods.back().get());
			if (r->is_default_initializer()) {
				has_default_initializer = true;
			}
		}
		else if (!check(Lexeme::End))
		{
	...
	// Create default initializer if none is found.
	if (!has_default_initializer)
	{
...
AutoAst Parser::parse_function_declaration(bool local, bool method)
{
	...
	if (method && token.spelling == init_string) {
		parsing_initializer.push_back(true);
	}
	else {
		parsing_initializer.push_back(false);
	}
	constexpr const char *hint = "in function declaration";
	auto name = parse_identifier(hint);
	expect(Lexeme::LParen, hint);
	auto params = parse_parameters(method);
...
AutoAst Parser::parse_return_statement()
{
	...
	if (token.is_separator())
	{
		if (!parsing_initializer.empty() && parsing_initializer.back()) {
			e = make<Variable>("this");
		}
	}
	else
	{
		if (!parsing_initializer.empty() && parsing_initializer.back()) {
			report_error("[Syntax error] Cannot return value from initializer");
		}
		e = parse_expression();
	}
