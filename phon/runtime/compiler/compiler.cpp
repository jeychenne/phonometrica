...
void Compiler::visit_integer(IntegerLiteral *node)
{
	...
	if (value >= (std::numeric_limits<Instruction>::min)() && value <= (std::numeric_limits<Instruction>::max)())
	{
		auto small_int = (Instruction) value;
		EMIT(Opcode::PushSmallInt, small_int);
	}
	else
	{
...
void Compiler::visit_assignment(Assignment *node)
{
	...
	// For self-assignment, we rewrite an expression such as "x += y" as "x = x + y".
...
void Compiler::visit_class_declaration(ClassDeclaration *node)
{
	auto ident = dynamic_cast<Variable*>(node->name.get());
	auto previous_class = current_class;
	current_class = ident->name;
	creating_ctor = true;
	Instruction name = routine->add_string_constant(current_class);
	static String object_class("Object");
	...
	{
		auto p = routine->add_string_constant(object_class);
		EMIT(Opcode::GetGlobal, p);
	}

	EMIT(Opcode::NewClass, name, Instruction(node->is_ref));
	//auto previous_scope = open_scope();

	// We now have a class object on the stack: add its fields
	...
	{
		auto index = add_local(current_class);
		EMIT(Opcode::SetLocal, index);
		getter = Opcode::GetLocal;
	}
	else
	{
		EMIT(Opcode::SetGlobal, name);
		getter = Opcode::GetGlobal;
	}
...
void Compiler::visit_routine(RoutineDefinition *node)
{
	...
	for (size_t i = 0; i < node->params.size(); i++)
	{
		...
		auto param = dynamic_cast<RoutineParameter*>(p.get());
	...
	{
		auto symbol = routine->resolve_upvalue(current_class, scope_depth);
		if (symbol)
		{
			EMIT(Opcode::GetUpvalue, *symbol);
		}
		else
		{
			auto cls = routine->add_string_constant(current_class);
			EMIT(Opcode::GetGlobal, cls);
		}
		EMIT(Opcode::NewInstance);
	}
	if (node->body)
	{
		node->body->visit(*this);
	...
	if (node->method)
	{
		auto name_id = routine->add_string_constant(name);
		EMIT(Opcode::PushString, name_id);
		return;
	}
	if (node->local || scope_depth > 1)
	{
...
void Compiler::visit_array(ArrayLiteral *node)
{
	...
	{
		THROW("Array literal can have at most % rows and % columns",
...
void Compiler::visit_index(IndexedExpression *node)
{
	...
	auto previous_assigned_lhs = visiting_assigned_lhs;
	visiting_assigned_lhs = false; // false for the indices
	for (auto &i : node->indexes) {
		i->visit(*this);
	}
	visiting_assigned_lhs = previous_assigned_lhs;
...
void Compiler::set_variable(Ast *node, const phonometrica::String &name)
{
	// Try to find a local variable, otherwise try to get a global.
	auto index = routine->find_local(name, scope_depth);
	if (index)
	{
		EMIT(Opcode::SetLocal, *index);
	}
	else if ((index = routine->resolve_upvalue(name, scope_depth)))
	{
		EMIT(Opcode::SetUpvalue, *index);
	}
	else
	{
		auto arg = routine->add_string_constant(name);
