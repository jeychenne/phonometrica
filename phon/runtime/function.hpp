...
#include <phon/runtime/compiler/code.hpp>
...
// and Routine, which is a user-defined function. Callables are an implementation detail and are not visible to users.
class Callable
{
public:
	...
	int get_cost(const std::vector<Class*> &args) const;

	String get_definition(bool is_method = false) const;
...
public:
	...
	struct Upvalue
	{
		...
		bool operator==(const Upvalue &other) const noexcept { return this->index == other.index && this->is_local == other.is_local; }
	};
...
public:
	...
	Handle<Function> copy(Runtime *rt);

private:

	struct CacheHasher {
		std::size_t operator()(const std::vector<Class*>& v) const
		{
			size_t hash = 0;
			for (auto ptr : v) {
				hash ^= std::hash<Class*>()(ptr) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
			}
...
Variant invoke_helper(Ret(*fn)(Args...), std::span<Variant> args, std::index_sequence<Is...>)
{
	return fn(cast<Args>(args[Is])...);
}

} // namespace detail

// Wrapper for native C++ functions.
template<typename Ret, typename... Args>
NativeCallback bind(Ret(*fn)(Args...)) {
	return [fn](Runtime& rt, std::span<Variant> args) -> Variant {
		if (unlikely(args.size() != sizeof...(Args)))
			throw error("[Internal error] Argument count mismatch in function wrapper");

		return detail::invoke_helper(fn, args, std::index_sequence_for<Args...>{});
	};
}
