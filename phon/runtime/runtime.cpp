...
void Runtime::create_builtins()
{
	...
	// Instance is a special type for instances of user-defined classes, so we don't add a global.
	create_type<Instance>("Instance", raw_object_class, Class::Index::Instance);
...
void Runtime::math_op(char op)
{
	...
	else if (check_type<List>(v1) && check_type<List>(v2) && op == '+')
	{
		List l1 = raw_cast<List>(v1); // make a deep copy
		auto &items = l1.items();
		for (auto &v : raw_cast<List>(v2)) {
			items.append(v);
		}
		pop(2);
		push(make_handle<List>(this, std::move(l1)));
		return;
	}

	auto cls1 = v1.class_name();
	auto cls2 = v2.class_name();
	pop(2);
	char opstring[2] = { op, '\0' };
	RUNTIME_ERROR("[Type error] Cannot apply math operator '%' to % and %", opstring, cls1, cls2);
}
...
{
	// Special flag to pass the newly created instance as the first argument of initialize().
	...
	while (true)
	{
		...
		switch (op)
		{
			...
			case Opcode::Assert:
			{
				...
				if (!value)
				{
					auto msg = (narg == 2) ? utils::format("Assertion failed: %", to_string(peek(-1))) : std::string("Assertion failed");
                    pop(narg);
					RUNTIME_ERROR(msg);
				}
                pop(narg);
			...
			case Opcode::Call:
			{
				...
				if (is_initializer)
				{
					++narg; // `this` was left on the stack
					is_initializer = false; // no longer needed
					calling_method = true;
				}
			...
			case Opcode::NewClass:
			{
				trace_op();
				const int index = *ip++;
				const bool is_ref = *ip++;
				auto &v = peek();
				if (!check_type<Class>(v)) {
					RUNTIME_ERROR("Expected a Class object as parent, got a %", v.get_class()->name());
				}
				auto &parent = raw_cast<Class>(v);
				String value = routine.get_string(index);
				auto klass = create_dynamic_type(std::move(value), &parent);

				if (is_ref) {
					klass->make_unclonable();
				}
				push(std::move(klass));
				break;
			}
			...
			case Opcode::NewField:
			{
				trace_op();
				auto &klass = raw_cast<Class>(peek(-3));
				auto field = raw_cast<String>(peek(-1));
				klass.set_instance_field(field, std::move(peek(-2)));
				pop(2); // leave class on the stack since it's being initialized
				break;
			}
			...
			case Opcode::NewInstance:
			{
				trace_op();
				auto &v = peek();
				auto instance = make_handle<Instance>(this, v.handle<Class>());
				auto &klass = raw_cast<Class>(v);
				klass.initialize_instance(*instance);
				pop();
			...
			case Opcode::NewMethod:
			{
				trace_op();
				auto name = raw_cast<String>(peek(-1));
				auto method = peek(-2).handle<Function>();
				auto &klass = raw_cast<Class>(peek(-3));
				klass.add_method(name, std::move(method));
			...
			case Opcode::Precall:
			{
				...
				else if (check_type<Class>(v))
				{
					...
					// Replace the class with its constructor, which will leave a (pre-)initialized instance on the stack.
					// For user-defined types, we then call the initializer.
					try
					{
						// TODO: call parent's constructor
						func = cls->get_constructor();

						if (cls->is_user_defined())
						{
							std::span<Variant> args;
							auto m = func->find_closure(args);
							auto instance = call_method(m, args);
							auto init = cls->get_initializer();
							// The constructor leaves a new instance on the stack, so we push the initializer and
							// the new instance, which appears as the first argument of the method (`this`).
							push(std::move(init));
							push(std::move(instance));
							is_initializer = true;
						}
			...
			case Opcode::SetField:
			{
				...
				if (check_type<Instance>(v))
				{
					auto &instance = raw_cast<Instance>(v);
					auto key = peek(-2).as_string();
					instance.set_field(key, peek(-1));
				}
				else
				{
...
size_t Runtime::disassemble_instruction(const Routine &routine, size_t offset)
{
	...
	switch (op)
	{
		...
		{
			int index = routine.code[offset + 1];
			int is_ref = routine.code[offset + 2];
			auto klass = routine.get_string(index);
			printf("NEW_CLASS      %-5d %d      ; %s\n", index, is_ref, klass.data());
		...
		case Opcode::NewField:
		{
			return print_simple_instruction("NEW_FIELD");
		}
		case Opcode::NewFrame:
		{
		...
		case Opcode::NewInstance:
		{
			return print_simple_instruction("NEW_INSTANCE");
		}
		case Opcode::NewIterator:
		{
		...
		case Opcode::NewMethod:
		{
			return print_simple_instruction("NEW_METHOD");
		}
		case Opcode::NewSet:
		{
			int nlocal = routine.code[offset+1];
...
void Runtime::push_call_frame(TObject<Closure> *closure, int nlocal)
{
	...
	// Is this still the case with stack allocated frames.
...
Variant Runtime::pop_call_frame()
{
	...
	{
		// Pop values on the stack + function which is also on the stack.
		pop(++n);
	}
	else
	{
		// Leave the locals because they are managed by the calling function which will pop them.
...
void Runtime::get_field(bool by_ref)
{
	...
	{
		auto &instance = raw_cast<Instance>(v);
		result = instance.get_field(peek().as_string(), by_ref);
	}
	else
	{
...
{
	Array<String> types;
	String routine = calling_method ? "method" : "function";
	...
	for (auto &c : func.closures) {
		candidates.append(c->routine->get_definition(calling_method));
		candidates.append('\n');
	}
	RUNTIME_ERROR("Cannot resolve call to % '%' with the following argument types: (%).\nCandidates are:\n%",
				  routine, func.name(), String::join(types, ", "), candidates);
}
...
{
	auto klass = make_handle<Class>(this, std::move(name), parent, &typeid(Instance), Class::Index::Instance);
	klass->set_object(klass.object());

	return klass;
}

String Runtime::to_string(const Variant &v)
{
	if (check_type<Instance>(v))
	{
		auto &i = raw_cast<Instance>(v);
		if (auto f = i.get_class()->get_to_string_method())
		{
			std::span<Variant> args(const_cast<Variant*>(&v), 1);
			auto closure = f->find_closure(args);
			assert(closure);
			auto result = call_method(closure, args);
