...
#include <phon/runtime/func_number.hpp>
#include <phon/runtime/func_integer.hpp>
...
static Variant integer_from_object(Runtime &, std::span<Variant> args)
{
...
static Variant float_from_object(Runtime &, std::span<Variant> args)
{
...
void Runtime::set_global_namespace()
{
	...
	bool_class->add_constructor(boolean_init, {});
	auto int_class = Class::get<intptr_t>();
	int_class->add_constructor(integer_init, {});
	int_class->add_constructor(integer_from_object, { CLS(Object) });
	auto float_class = Class::get<double>();
	float_class->add_constructor(float_init, {});
	float_class->add_constructor(float_from_object, { CLS(Object) });

	// Generic functions
	add_global("type", get_type, { CLS(Object) });
	add_global("len", get_length, { CLS(Object) });
	add_global("str", to_string_func, { CLS(Object) });
	...
	module_class->add_constructor(module_init, {CLS(String)});
	...
	string_class->add_constructor(string_init, {});
	string_class->add_constructor(string_from_object, { CLS(Object) });
	...
	list_class->add_constructor(list_init, {});
	...
	auto &v = (*globals)["open"];
	file_class->add_constructor(v.handle<Function>());
	...
	table_class->add_constructor(table_init, {});
	...
	array_class->add_constructor(zeros.handle<Function>());
	...
	// Number
	auto number_class = Class::get<Number>();
	number_class->add_constructor(number_new, {CLS(String)});

	// Integer
	auto integer_class = Class::get<intptr_t>();
	...
	set_class->add_constructor(set_init, {});
	...
	add_global("get_error_trace", system_error_trace, {});
