...
PitchQueryEditor::PitchQueryEditor(QWidget *parent) :
	PitchQueryEditor(Handle<PitchQuery>::make(nullptr, String()), parent) {}
...
QWidget *PitchQueryEditor::createPitchSettingsPanel()
{
	...
	// Disclosure: per-property pitch-range override
...
// ── Per-property pitch-range override section ────────────────────────────────
// Mirrors the formant editor's disclosure-triangle pattern. Expanding the triangle
// only toggles panel visibility — the override is active iff there are pending
// entries with at least one non-zero value. The global Min/Max pitch fields are
// disabled per-field when fully covered.

QWidget *PitchQueryEditor::buildOverrideSection()
{
	...
	m_override_triangle->setText(tr("Adjust pitch range by property"));
	m_override_triangle->setAutoRaise(true);
	m_override_triangle->setToolTip(tr(
		"Override the pitch range on a per-file basis using the value of a "
		"text-typed property (e.g. Gender). When the override is active, the "
		"global Min/Max pitch fields above are disabled per-field once every "
	...
	// Show/hide the per-match Min/Max pitch columns on the resulting concordance.
	...
	m_show_params_check->setToolTip(tr(
		...
		"effective Min pitch and Max pitch values as extra columns. You can "
		"show or hide these columns later via the concordance's Display menu."));
...
void PitchQueryEditor::setOverrideExpanded(bool expanded)
{
...
void PitchQueryEditor::applyOverrideEnabledState()
{
	// Per-field disable: each global field is disabled iff every known level has
	// an override for that field. Independent because users may override only
	// the floor or only the ceiling.
	int rows_min_covered = 0;
	int rows_max_covered = 0;
	for (auto &entry : m_pending_overrides) {
		if (entry.second.min_pitch > 0) ++rows_min_covered;
		if (entry.second.max_pitch > 0) ++rows_max_covered;
	}
	...
	if (m_min_pitch_edit) m_min_pitch_edit->setEnabled(!min_fully_covered);
	if (m_max_pitch_edit) m_max_pitch_edit->setEnabled(!max_fully_covered);
}

void PitchQueryEditor::refreshOverrideCategoryCombo()
{
...
void PitchQueryEditor::refreshOverrideTable()
{
	...
	QString hint = tr("Defaults: min %1 Hz, max %2 Hz (leave a cell blank to inherit)")
		.arg(m_min_pitch_edit ? m_min_pitch_edit->text() : QStringLiteral("?"))
		.arg(m_max_pitch_edit ? m_max_pitch_edit->text() : QStringLiteral("?"));
	...
	m_override_table->setHorizontalHeaderLabels(QStringList{
		tr("Value"), tr("Min pitch (Hz)"), tr("Max pitch (Hz)")
	});
	...
	for (const auto &v : values)
	{
		...
		PitchQuery::LevelOverride ov;
		auto it = m_pending_overrides.find(v);
		if (it != m_pending_overrides.end()) ov = it->second;

		m_override_table->setItem(row, 1, new QTableWidgetItem(format_cell(ov.min_pitch)));
		m_override_table->setItem(row, 2, new QTableWidgetItem(format_cell(ov.max_pitch)));
		++row;
	}
...
void PitchQueryEditor::syncOverrideCellToCache(int row, int col)
{
	...
	if      (col == 1) entry.min_pitch = parsed;
	else if (col == 2) entry.max_pitch = parsed;

	if (entry.min_pitch <= 0 && entry.max_pitch <= 0) {
...
void PitchQueryEditor::updateOverrideStatus()
{
	...
	// Count rows with at least one effective override.
	int rows_any = 0;
	int rows_min = 0;
	int rows_max = 0;
	for (int r = 0; r < row_count; r++) {
		...
		bool has_min = ov.min_pitch > 0;
		bool has_max = ov.max_pitch > 0;
...
void PitchQueryEditor::parseQuery()
{
	...
	// Per-property pitch-range override: active iff at least one pending entry
	// has a non-zero value. Triangle visibility is independent — a collapsed
	// panel with values still applies.
	m_query->clear_override_levels();
	bool has_any_override = false;
	for (auto &entry : m_pending_overrides) {
		const auto &ov = entry.second;
		if (ov.min_pitch > 0 || ov.max_pitch > 0) {
...
void PitchQueryEditor::onExecute()
{
	...
	QueryProgressDialog progress(*m_query, tr("Measuring pitch..."), this);
...
void PitchQueryEditor::onSaveAs()
{
	...
	auto path = getSaveFileName(this, tr("Save pitch query..."),
...
void PitchQueryEditor::loadQuery()
{
	...
	// Per-property pitch-range override
