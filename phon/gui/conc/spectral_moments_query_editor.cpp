...
{
	setWindowTitle(tr("Measure spectral moments..."));
...
void SpectralMomentsQueryEditor::setupUi()
{
	...
	left_layout->addWidget(createSpectralMomentsSettingsPanel());
...
QWidget *SpectralMomentsQueryEditor::createSearchPanel()
{
	...
	connect(first, &ConstraintWidget::searchRequested, this, &SpectralMomentsQueryEditor::onExecute);
	...
	connect(m_add_btn, &QPushButton::clicked, this, &SpectralMomentsQueryEditor::onAddConstraint);
	connect(m_remove_btn, &QPushButton::clicked, this, &SpectralMomentsQueryEditor::onRemoveConstraint);
	return group;
}

QWidget *SpectralMomentsQueryEditor::createSpectralMomentsSettingsPanel()
{
	auto *group = new QGroupBox(tr("Spectral moments"));
	auto *outer = new QVBoxLayout(group);

	// ── Analysis parameters ──────────────────────────────────────────────
	auto *param_row1 = new QHBoxLayout;
	param_row1->addWidget(new QLabel(tr("Window (s):")));
	m_window_duration_edit = new QLineEdit("0.025");
	m_window_duration_edit->setFixedWidth(70);
	m_window_duration_edit->setToolTip(tr("Analysis window duration in seconds (default 25 ms for fricatives)"));
	param_row1->addWidget(m_window_duration_edit);

	param_row1->addSpacing(10);
	param_row1->addWidget(new QLabel(tr("Window type:")));
	m_window_type_combo = new QComboBox;
	m_window_type_combo->addItems({"Gaussian", "Hamming", "Hann", "Blackman", "Bartlett", "Rectangular"});
	m_window_type_combo->setCurrentIndex(0);
	m_window_type_combo->setFixedWidth(110);
	param_row1->addWidget(m_window_type_combo);
	param_row1->addStretch();
	outer->addLayout(param_row1);

	auto *param_row2 = new QHBoxLayout;
	param_row2->addWidget(new QLabel(tr("Min freq (Hz):")));
	m_min_freq_edit = new QLineEdit("0");
	m_min_freq_edit->setFixedWidth(70);
	param_row2->addWidget(m_min_freq_edit);

	param_row2->addSpacing(10);
	param_row2->addWidget(new QLabel(tr("Max freq (Hz):")));
	m_max_freq_edit = new QLineEdit("0");
	m_max_freq_edit->setFixedWidth(70);
	m_max_freq_edit->setToolTip(tr("0 = Nyquist frequency"));
	param_row2->addWidget(m_max_freq_edit);

	param_row2->addSpacing(10);
	m_preemph_check = new QCheckBox(tr("Pre-emphasis (Hz):"));
	m_preemph_check->setChecked(true);
	m_preemph_check->setToolTip(tr("Apply 6 dB/octave pre-emphasis to compensate for spectral tilt"));
	param_row2->addWidget(m_preemph_check);
	m_preemph_edit = new QLineEdit("50");
	m_preemph_edit->setFixedWidth(50);
	m_preemph_edit->setToolTip(tr("Pre-emphasis threshold frequency in Hz"));
	param_row2->addWidget(m_preemph_edit);
	param_row2->addStretch();
	outer->addLayout(param_row2);

	connect(m_preemph_check, &QCheckBox::toggled, m_preemph_edit, &QLineEdit::setEnabled);

	// ── Moment selection ────────────────────────────────────────────────
	auto *moment_row = new QHBoxLayout;
	moment_row->addWidget(new QLabel(tr("Output:")));
	m_cog_check = new QCheckBox(tr("COG"));
	m_spread_check = new QCheckBox(tr("Spread"));
	m_skewness_check = new QCheckBox(tr("Skewness"));
	m_kurtosis_check = new QCheckBox(tr("Kurtosis"));
	m_cog_check->setChecked(true);
	m_spread_check->setChecked(true);
	m_skewness_check->setChecked(true);
	m_kurtosis_check->setChecked(true);
	moment_row->addWidget(m_cog_check);
	moment_row->addWidget(m_spread_check);
	moment_row->addWidget(m_skewness_check);
	moment_row->addWidget(m_kurtosis_check);
	moment_row->addStretch();
	outer->addLayout(moment_row);
...
QWidget *SpectralMomentsQueryEditor::createContextPanel()
{
...
QWidget *SpectralMomentsQueryEditor::createFileSelector()
{
...
QWidget *SpectralMomentsQueryEditor::createMetadataPanel()
{
...
QWidget *SpectralMomentsQueryEditor::createButtonPanel()
{
	...
	connect(ok_btn, &QPushButton::clicked, this, &SpectralMomentsQueryEditor::onExecute);
	connect(cancel_btn, &QPushButton::clicked, this, &QDialog::reject);
	connect(m_save_btn, &QPushButton::clicked, this, &SpectralMomentsQueryEditor::onSave);
	connect(m_save_as_btn, &QPushButton::clicked, this, &SpectralMomentsQueryEditor::onSaveAs);
	return widget;
}

void SpectralMomentsQueryEditor::parseQuery()
{
	m_query->clear();
	...
	// ── Analysis parameters ─────────────────────────────────────────────
	bool ok;
	double win_dur = m_window_duration_edit->text().toDouble(&ok);
	if (!ok || win_dur <= 0) throw std::runtime_error("Invalid window duration (must be > 0)");
	m_query->set_window_duration(win_dur);

	// Window type
	QString wt = m_window_type_combo->currentText();
	if (wt == "Hann")              m_query->set_window_type(speech::WindowType::Hann);
	else if (wt == "Gaussian")     m_query->set_window_type(speech::WindowType::Gaussian);
	else if (wt == "Blackman")     m_query->set_window_type(speech::WindowType::Blackman);
	else if (wt == "Bartlett")     m_query->set_window_type(speech::WindowType::Bartlett);
	else if (wt == "Rectangular")  m_query->set_window_type(speech::WindowType::Rectangular);
	else                           m_query->set_window_type(speech::WindowType::Hamming);

	double min_f = m_min_freq_edit->text().toDouble(&ok);
	if (!ok || min_f < 0) throw std::runtime_error("Invalid minimum frequency");
	m_query->set_min_frequency(min_f);

	double max_f = m_max_freq_edit->text().toDouble(&ok);
	if (!ok || max_f < 0) throw std::runtime_error("Invalid maximum frequency");
	m_query->set_max_frequency(max_f);

	m_query->set_use_preemphasis(m_preemph_check->isChecked());
	if (m_preemph_check->isChecked()) {
		double pe = m_preemph_edit->text().toDouble(&ok);
		if (!ok || pe < 0) throw std::runtime_error("Invalid pre-emphasis threshold");
		m_query->set_preemphasis(pe);
	}

	// Moment selection (at least one must be checked)
	m_query->set_output_cog(m_cog_check->isChecked());
	m_query->set_output_spread(m_spread_check->isChecked());
	m_query->set_output_skewness(m_skewness_check->isChecked());
	m_query->set_output_kurtosis(m_kurtosis_check->isChecked());
	if (!m_cog_check->isChecked() && !m_spread_check->isChecked() &&
	    !m_skewness_check->isChecked() && !m_kurtosis_check->isChecked())
	{
		throw std::runtime_error("At least one spectral moment must be selected");
	}

	// Measurement location
	if (m_midpoint_radio->isChecked()) {
		m_query->set_method(SpectralMomentsQuery::Method::Midpoint);
	} else {
		m_query->set_method(SpectralMomentsQuery::Method::NPoint);
...
bool SpectralMomentsQueryEditor::validateQuery()
{
...
void SpectralMomentsQueryEditor::onExecute()
{
...
void SpectralMomentsQueryEditor::onSave()
{
...
void SpectralMomentsQueryEditor::onSaveAs()
{
	auto path = getSaveFileName(this, tr("Save spectral moments query..."), tr("Phonometrica query (*.phon-query)"));
...
void SpectralMomentsQueryEditor::onAddConstraint()
{
...
void SpectralMomentsQueryEditor::onRemoveConstraint()
{
...
void SpectralMomentsQueryEditor::loadQuery()
{
	...
	// Analysis parameters
	m_window_duration_edit->setText(QString::number(m_query->window_duration(), 'f', 4));
	// Set window type combo
	auto set_window_combo = [&](speech::WindowType wt) {
		switch (wt) {
			case speech::WindowType::Hann:        m_window_type_combo->setCurrentText("Hann"); break;
			case speech::WindowType::Gaussian:    m_window_type_combo->setCurrentText("Gaussian"); break;
			case speech::WindowType::Blackman:    m_window_type_combo->setCurrentText("Blackman"); break;
			case speech::WindowType::Bartlett:    m_window_type_combo->setCurrentText("Bartlett"); break;
			case speech::WindowType::Rectangular: m_window_type_combo->setCurrentText("Rectangular"); break;
			default:                              m_window_type_combo->setCurrentText("Hamming"); break;
		}
	};
	set_window_combo(m_query->window_type());

	m_min_freq_edit->setText(QString::number(m_query->min_frequency(), 'f', 0));
	m_max_freq_edit->setText(QString::number(m_query->max_frequency(), 'f', 0));
	m_preemph_check->setChecked(m_query->use_preemphasis());
	m_preemph_edit->setText(QString::number(m_query->preemphasis(), 'f', 0));
	m_preemph_edit->setEnabled(m_query->use_preemphasis());

	// Moment selection
	m_cog_check->setChecked(m_query->output_cog());
	m_spread_check->setChecked(m_query->output_spread());
	m_skewness_check->setChecked(m_query->output_skewness());
	m_kurtosis_check->setChecked(m_query->output_kurtosis());
