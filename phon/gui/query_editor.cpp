...
QueryEditor::QueryEditor(QWidget *parent) :
	QDialog(parent), m_query(make_handle<Query>(nullptr, String()))
{
	...
	QDialog(parent), m_query(std::move(query))
{
	setWindowTitle(tr("Find in annotations..."));
	setMinimumSize(800, 600);
	m_query->open(); // Ensure file is loaded from disk if not already.
	setupUi();
	loadQuery();
}
...
QWidget *QueryEditor::createSearchPanel()
{
	...
	m_remove_btn = new QPushButton(tr("\u2212")); // minus sign
...
void QueryEditor::onSave()
{
	...
	m_query->set_content_modified(true);
	m_query->save();
}
