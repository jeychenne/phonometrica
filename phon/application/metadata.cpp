...
Version::Version(Date date, const String &description, const String &number) :
...
String Version::date_as_string() const
{
    return String::format("%-%-%",
                          (int)m_date.year(),
                          (unsigned)m_date.month(),
                          (unsigned)m_date.day());
}

Date Version::parse_date(const String &str)
{
...
void Changelog::from_xml(xml_node root)
{
	...
	for (auto node = root.first_child(); node; node = node.next_sibling())
	{
        ...
        auto date = Version::parse_date(date_node.text().get());
