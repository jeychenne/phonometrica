...
void AGraph::remove_interval(intptr_t layer_index, double start, double end)
{
	double t = start + (end-start) / 2;
	auto &layer = m_layers.at(layer_index);
	auto it = std::lower_bound(layer->events.begin(), layer->events.end(), t, EventLessEqual());

	if (it != layer->events.end()) {
		layer->events.remove(*it);
	}
	set_modified(true);
}

void AGraph::remove_events(intptr_t layer_index)
{
	m_layers.at(layer_index)->events.clear();
	set_modified(true);
}
